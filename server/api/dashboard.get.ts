...
export default defineEventHandler(async (event) => {
  ...
  const outstandingRow = db.select({
  ...
  const overduePayments = db.select(paymentFields)
    .from(schema.payments)
  ...
  const readyToInvoice = db.select(paymentFields)
    .from(schema.payments)
    .leftJoin(schema.projects, eq(schema.payments.projectId, schema.projects.id))
    .leftJoin(schema.clients, eq(schema.projects.clientId, schema.clients.id))
  ...
  const dueThisWeek = db.select(paymentFields)
    .from(schema.payments)
    .leftJoin(schema.projects, eq(schema.payments.projectId, schema.projects.id))
    .leftJoin(schema.clients, eq(schema.projects.clientId, schema.clients.id))
